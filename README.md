...
1. UIのブラッシュアップ
    ...
    検索結果のリポジトリの各セルのデザインを変更しました。

    リポジトリ検索中のダイアログを追加しました。

    画面横向き用のUI、iPad用のUIを作成しました。

2. 機能追加

    リポジトリの詳細画面にてリポジトリのリンクをブラウザで開ける機能を追加しました。

    リポジトリの詳細画面にてリポジトリのリンクを共有できる機能を追加しました。

    リポジトリの検索画面にて検索バーの文字列をクリアすることで検索結果もクリアされるように変更しました。
